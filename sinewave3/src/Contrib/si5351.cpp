...
#else // WISP
	uint8_t	localaddr = addr;
	uint8_t *params = new uint8_t[20];
	int	i;

#ifdef CFG_DEBUG_SI5351
	{
		int	j;
		uint8_t *tempdata = data;
		trace_printf("write_bulk: %02d:", addr);
		for (j=0; j< bytes; j++) {
			trace_printf("%02x ", *tempdata++);
		}
		trace_printf("\n");
	}
#endif //CFG_DEBUG_SI5351
...
#ifdef CFG_USEDEVICEREADY
    if (HAL_I2C_IsDeviceReady(&Globals.I2cHandle, (uint16_t)SI5351_BUS_BASE_ADDR<<1, 2, 2000) != HAL_OK) {
    	Error_Handler();
    }
#endif //USEDEVICEREADY

	while(HAL_I2C_Master_Transmit(&Globals.I2cHandle, (uint16_t)SI5351_BUS_BASE_ADDR<<1, (uint8_t *)params, bytes+1, 10000)!= HAL_OK)
	{
...
#endif //USEDEVICEREADY
	while(HAL_I2C_Master_Transmit(&Globals.I2cHandle, (uint16_t)SI5351_BUS_BASE_ADDR<<1, (uint8_t*)&buffer, sizeof(buffer), 10000)!= HAL_OK)
	{
		/* Error_Handler() function is called when Timeout error occurs.
	       When Acknowledge failure occurs (Slave don't acknowledge it's address)
	       Master restarts communication */
...
#endif //USEDEVICEREADY
	while(HAL_I2C_Master_Receive(&Globals.I2cHandle, (uint16_t)SI5351_BUS_BASE_ADDR<<1, (uint8_t *)&value, 1, 10000) != HAL_OK)
	{
		/* Error_Handler() function is called when Timeout error occurs.
       	   When Acknowledge failure occurs (Slave don't acknowledge it's address)
       	   Master restarts communication */
